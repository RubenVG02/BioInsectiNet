...
def load_or_initialize_epoch_log(subsets, log_file):
    ...
    if not os.path.exists(log_path):
        ...
        log_info(f"Epoch log file created at {log_path}")
...
def train_subsets(subset, models_dir, epochs, batch_size, learning_rate, log_file, data_dir):
    ...
    log_info(f"Training model for subset {subset}...")
    ...
    log_success(f"Model for subset {subset} trained successfully!")
...
def main():
    ...
    if args.data_dir is None or not os.path.exists(args.data_dir):
        log_warning("No data directory specified or the directory does not exist. Please select a valid directory.")
        args.data_dir = select_dir()
        log_info(f"Selected directory: {args.data_dir}")
    ...
    if args.models_dir is None:
        log_warning(f"No models directory specified. Using default models/generator/{os.path.basename(args.data_dir)}")
        args.models_dir = f"models/generator/{os.path.basename(args.data_dir)}"
    os.makedirs(args.models_dir, exist_ok=True)
    log_info(f"Models will be saved in: {args.models_dir}")
    ...
    log_info(f"Epoch log file: {args.log_file}")
    log_info(f"Epoch log loaded with {len(epoch_log)} subsets.")
    for subset in subsets:
        ...
        if epoch_log[subset][0] >= args.epochs or epoch_log[subset][1] == True: # If the model has already been trained for the specified number of epochs or if early stopping was triggered (therefore, the model is already trained).
            log_warning(f"Skipping training for subset {subset}. Already trained for {epoch_log[subset][0]} epochs or early stopping was triggered.")
            continue
    ...
    log_success("All subsets processed. Training complete.")
