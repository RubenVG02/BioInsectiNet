...
from rdkit.Chem import Descriptors, Lipinski, Draw, FilterCatalog, QED
...
def parse_arguments():
    ...
    parser.add_argument("--json_file", type=str, default="models/unique_chars_dict.json", help="Path to the JSON file containing the unique characters dictionary.")
...
def check_pains(smiles):
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        return True  # Consider it PAINS if invalid
    entry = pains_catalog.GetFirstMatch(mol)
    return entry is not None

def check_additional_properties(smiles):
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        return False
...
def generate_druglike_molecules(model_path, char_to_idx, vocab_size, num_molecules=100, min_length = 30, max_length=100, temperature=1.0, save_images=False, output_dir="generated_molecules"):
    ...
    with tqdm(total=num_molecules, desc="Generating molecules") as pbar:
        while len(druglike_molecules) - len(previous_molecules) < num_molecules:            
            ...
            has_additional_properties = check_additional_properties(smiles)
            if not is_druglike_flag or is_pains or not has_additional_properties or len(smiles) < min_length or len(smiles) > max_length:
                continue 
...
def get_filename_regex_json(json_path, base_name):
